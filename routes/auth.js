...
router.post('/refresh', async (req, res) => {
  ...
  try {
    ...
    const newRefreshToken = await rotateRefreshToken(prisma, validUser.id);
    const tokens = issueTokens(validUser, { refreshToken: newRefreshToken });
    setRefreshCookie(res, newRefreshToken);

    return res.json({
      user: sanitizeUser(validUser),
      accessToken: tokens.accessToken,
      accessTokenExpiresInMs: tokens.accessTokenExpiresInMs,
      refreshTokenExpiresInMs: tokens.refreshTokenExpiresInMs
    });
    ...
    return res.status(500).json({ error: 'Не удалось обновить токен. Попробуйте позже.' });
  }
});
