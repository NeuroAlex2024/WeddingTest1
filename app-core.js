  ...
  const App = {
    ...
    async handleAuthSubmit(form) {
      ...
      try {
        ...
        console.log('[Auth] Регистрация успешна, обновляем состояние...');
        this.refreshAuthState();
        console.log('[Auth] Загружаем профиль с сервера...');
        try {
          await this.refreshProfileFromServer({ silent: true, persist: true });
          console.log('[Auth] Профиль загружен успешно');
        } catch (profileError) {
          console.error('[Auth] Ошибка загрузки профиля:', profileError);
          throw profileError;
        }
        if (this.state.currentRole === 'wedding') {
          console.log('[Auth] Загружаем каталог подрядчиков...');
          try {
            await this.fetchMarketplaceCatalog({ force: true });
            console.log('[Auth] Каталог загружен успешно');
          } catch (catalogError) {
            console.error('[Auth] Ошибка загрузки каталога:', catalogError);
            throw catalogError;
          }
        }
        console.log('[Auth] Закрываем модальное окно...');
        this.closeModal();
        if (location.hash !== '#/dashboard') {
          console.log('[Auth] Переходим на /dashboard...');
          location.hash = '#/dashboard';
        }
        console.log('[Auth] Рендерим интерфейс...');
        this.render();
        console.log('[Auth] Авторизация полностью завершена!');
      } catch (error) {
        console.error('[Auth] ОШИБКА при авторизации:', error);
        console.error('[Auth] Тип ошибки:', error.constructor.name);
        console.error('[Auth] Сообщение:', error.message);
        console.error('[Auth] Стек:', error.stack);
        if (errorEl) {
          errorEl.textContent = 'Произошла ошибка: ' + error.message;
          errorEl.dataset.status = 'error';
        }
      } finally {
        if (submitButton) {
